...
export async function createPostPaymentArtifacts(paymentId: string) {
  ...
  const { data: registration } = await admin.from('registrations').select('id,registration_number,event_id,category_id,first_name,last_name,email').eq('id', payment.registration_id).maybeSingle();
  ...
  await admin.from('email_messages').upsert({ recipient_email: registration.email, recipient_user_id: null, organization_id: payment.organization_id, event_id: event.id, registration_id: registration.id, payment_id: payment.id, receipt_id: receipt?.id ?? null, email_type: 'registration_confirmed', template_version: 1, template_snapshot: { type: 'registration_confirmed' }, subject: `Registration Confirmed — ${event.name}`, rendered_body_reference: qr.dataUrl, status: 'queued', idempotency_key: `registration-confirmed:${registration.id}` }, { onConflict: 'idempotency_key' });
}
