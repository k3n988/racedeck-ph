...
export async function POST(request: Request) {
  ...
  const documentType = form.get('document_type') === 'government_id' ? 'government_id' : 'supporting_document';
  ...
  const existing = await admin.from('organizer_verifications').select('id,status,document_storage_references').eq('organization_id', membership.organization_id).maybeSingle();
  if (!existing.data) return NextResponse.json({ error: 'Save verification details before uploading documents' }, { status: 400 });
  ...
  const safeName = file.name.replace(/[^a-zA-Z0-9._-]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 120) || `document.${extension}`;
  const path = `${membership.organization_id}/${existing.data.id}/${crypto.randomUUID()}-${safeName}`;
  ...
  const reference = { path, name: file.name.slice(0, 200), document_type: documentType, content_type: file.type, size: file.size, uploaded_at: new Date().toISOString() };
